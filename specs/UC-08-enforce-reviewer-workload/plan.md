...
**Branch**: `UC-08-enforce-reviewer-workload` | **Date**: 2026-02-05 | **Spec**: /Users/chathilaratnatilake/Documents/University/WINTER 2026/ECE 493/ECE-493-CMS/specs/UC-08-enforce-reviewer-workload/spec.md
**Input**: Feature specification from `/specs/UC-08-enforce-reviewer-workload/spec.md`

## Summary

Enforce reviewer workload limits by retrieving current assignment counts,
blocking assignments at the configured limit (default 5), and updating workload
counts on success. Missing or failed workload checks block assignments with clear
errors.
...
**Project Type**: web  
**Performance Goals**: Workload check completes within 2 seconds  
...
- Requirements-first, traceable outputs: PASS (spec derived from US-08 and
  repository acceptance tests).
- Use-case fidelity: PASS (flows preserved; no control-flow changes).
- One feature per use case: PASS (branch `UC-08-enforce-reviewer-workload`
  dedicated to UC-08).
- No manual patching of generated artifacts: PASS (artifacts generated via
  workflow).
- Scope boundaries (from CMS SRS): PASS (workload limit enforcement is in scope).
- Architecture and implementation constraints: PASS (MVC + vanilla HTML/CSS/JS,
  relational DB abstraction; Java backend).
- Security and reliability: PASS (clear validation feedback, error handling for
  retrieval failures).
...
```text
specs/UC-08-enforce-reviewer-workload/
