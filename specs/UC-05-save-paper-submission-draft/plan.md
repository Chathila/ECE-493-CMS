...
**Branch**: `UC-05-save-paper-submission-draft` | **Date**: 2026-02-05 | **Spec**: /Users/chathilaratnatilake/Documents/University/WINTER 2026/ECE 493/ECE-493-CMS/specs/UC-05-save-paper-submission-draft/spec.md
**Input**: Feature specification from `/specs/UC-05-save-paper-submission-draft/spec.md`

## Summary

Allow logged-in authors to save the current submission state as a draft with
partial validation (minimum title + corresponding author email), store draft data
in the CMS database, and show confirmation. No-change saves should still confirm.
Invalid draft data or storage errors must surface clear error messages.
...
- Requirements-first, traceable outputs: PASS (spec derived from US-05 and
  repository acceptance tests).
- Use-case fidelity: PASS (flows preserved; no control-flow changes).
- One feature per use case: PASS (branch `UC-05-save-paper-submission-draft`
  dedicated to UC-05).
- No manual patching of generated artifacts: PASS (artifacts generated via
  workflow).
- Scope boundaries (from CMS SRS): PASS (draft saving is in scope).
...
```text
specs/UC-05-save-paper-submission-draft/
